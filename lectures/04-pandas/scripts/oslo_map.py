...
oslo_map = Map(
    basemap=basemap_to_tiles(basemaps.OpenStreetMap.Mapnik), center=oslo_center, zoom=13
)
...
start_pos = float(new_stations_data["start_station_latitude"]), float(
    new_stations_data["start_station_longitude"]
)
...
def add_lines(row):
    ...
    circle = Circle(
        location=end, color="red", radius=int(5 * (end_stations[row["end_station_id"]]))
    )
    circle_start = Circle(location=start, color="green", radius=10)
    oslo_map.add_layer(circle)
    oslo_map.add_layer(circle_start)
    circle.popup = HTML(
        f"{row['end_station_name']} <br> Trips ended: {end_stations[row['end_station_id']]}"
    )
