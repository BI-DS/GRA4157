...
def c4(goals):  # This task has multiple solutions!
    ...
    pos = {
        ...
        "HB": 2,
        "FW": 3,
    }  # I have a mapping to determine where in the team-list players should be
    n = len(goals)
    for i in range(n):
        player = goals.iloc[i]  # Find the Series for the player
        p = player["Pos."]["Pos."].split()[
            0
        ]  # Find position of player. Here i chose the first position ([0]) if the player has multiple positions.
        idx = pos[p]  # Index from pos dictionary
        if 0 in team[idx]:  # If there is still an idle spot in the players position
            count = team[idx].count(
                0
            )  # Find out which sub-position to play (midfield right, midfield left etc). Arbitrary
